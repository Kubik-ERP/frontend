    ...
    category.menus.forEach((menu, menuIndex) => {
      if (menu.subMenus?.length) {
        ...
        const isSubMenuActive = menu.subMenus.some(
          (subMenu) => subMenu.path === route.path
        );
...
const toggleSubMenu = (categoryIndex: number, menuIndex: number) => {
  ...
  expandedMenus.value[categoryIndex][menuIndex] =
    !expandedMenus.value[categoryIndex][menuIndex];
};
</script>
      ...
      <section id="outlet"
        ...
        <section id="outlet-information" class="flex flex-col">
          ...
          <p class="font-normal text-text-disabled text-[10px]">
            Jl. Kaliurang KM 13.5, No 17A
          </p>
        </section>
      </section>

      <div class="overflow-y-auto">
        <ul v-for="(menuCategory, menuCategoryIndex) in LIST_SIDEBAR_MENUS" :key="`menu-${menuCategoryIndex}`"
          class="list-none m-0">
          <li>
            <!-- Category Header (no chevron) -->
            <div v-ripple class="p-4 flex items-center text-surface-500 dark:text-surface-400 cursor-default">
            ...
            <ul class="list-none p-0 m-0 overflow-hidden">
              <li v-for="(menu, menuIndex) in menuCategory.menus" :id="`menu-${menuIndex}`" :key="`menu-${menuIndex}`">
                <template v-if="menu.subMenus?.length">
                  <a v-ripple
                    class="flex items-center cursor-pointer p-4 rounded duration-150 transition-colors p-ripple gap-2"
                    :class="[
                    ...
                    <span class="font-normal text-base" :class="[
                      menu.subMenus.some(subMenu => subMenu.path === route.path)
                        ? 'text-white'
                        : 'text-black',
                    ]">
                    ...
                    <i :class="[
                      'pi',
                      'ml-auto',
                      expandedMenus[menuCategoryIndex]?.[menuIndex]
                        ? 'pi-chevron-up'
                        : 'pi-chevron-down',
                    ]"></i>
                  </a>
                  <ul class="list-none p-0 m-0 overflow-hidden transition-max-height duration-400 ease-in-out" :class="{
                ...
                <template v-else>
                  <RouterLink v-ripple :to="menu.path"
                    ...
                    <p class="font-normal text-base" :class="[

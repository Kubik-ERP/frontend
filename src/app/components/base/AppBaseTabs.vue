...
<template>
  <PrimeVueTabs :value="localValue" @update:value="(value: string | number) => onChange(value)">
    <PrimeVueTabList :pt="{
      ...
      <PrimeVueTab v-for="(tab, tabIndex) in props.items" :key="`tab-${tabIndex}`" :value="tab.value" :pt="{
        root: `border-none text-sm rounded-lg max-h-10 py-2 ${dynamicWidth} ${localValue === tab.value
    ...
    <PrimeVueTabPanels v-for="(tabPanel, tabPanelIndex) in props.items" :key="`tab-panel-${tabPanelIndex}`" class="px-0"

      ...
      <form action="" class="grid-wrapper gap-4">
        <section id="form-group" class="flex flex-col col-span-full lg:col-span-6 gap-4">
          <AppBaseFormGroup v-slot="{ classes }" class-label="block text-sm font-medium leading-6 text-gray-900 w-full"
            is-name-as-label label-for="open-balance" name="Open Register Balance" spacing-bottom="mb-0"
            :validators="cashDrawer_formValidations.openBalance">
            <PrimeVueInputNumber v-model="cashDrawer_formData.openBalance" prefix="Rp " fluid :class="{
              ...classes,
            }" placeholder="Rp 0,00" />
          </AppBaseFormGroup>
        ...
        <section id="form-group" class="col-span-full lg:col-span-6">
          <AppBaseFormGroup v-slot="{ classes }" class-label="block text-sm font-medium leading-6 text-gray-900 w-full"
            is-name-as-label label-for="staff" name="Staff" spacing-bottom="mb-0"
            :validators="cashDrawer_formValidations.staffName">
            <PrimeVueInputText v-model="cashDrawer_formData.staffName" placeholder="" class="text-sm w-full"
              :class="{ ...classes }" />
          </AppBaseFormGroup>
        </section>
        ...
        <section id="form-group" class="col-span-full">
          <AppBaseFormGroup v-slot="{ classes }" class-label="block text-sm font-medium leading-6 text-gray-900 w-full"
            ...
            <PrimeVueIconField>
              <PrimeVueTextarea v-model="cashDrawer_formData.notes" placeholder="Describe open register notes here"
    ...
    <template #footer>
      <footer class="flex items-center justify-end w-full gap-4">
        <PrimeVueButton
          ...
          label="Cancel" severity="secondary" variant="outlined" @click="cashDrawer_onCloseRegister" />

        <PrimeVueButton class="bg-blue-primary border-none text-base py-[10px] w-full max-w-40" label="Save"
          type="button" :disabled="cashDrawer_formValidations.$invalid" />
      </footer>
    </template>
  </AppBaseDialog>
</template>
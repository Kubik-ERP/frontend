...
import { useCashInOutListService } from '@/modules/cash-in-out/services/cash-in-out-list.service';
...
export const useSalesOrderService = (): ISalesOrderProvided => {
  ...
  const { cashInOutList_fetchListTransactions } = useCashInOutListService();
  ...
  watch(
    ...
    async newTab => {
      switch (newTab.toUpperCase()) {
        ...
        case 'CASH-IN-OUT': {
          await cashInOutList_fetchListTransactions();

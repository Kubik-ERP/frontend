...
const { voucherView_voucher, voucherView_fetchVoucher, voucherView_handleEdit } = useVoucherViewService();
...
<template>
  <div class="voucher-view-ui flex justify-center items-start mt-5 max-w-7xl mx-auto px-4 sm:px-6">
    <div class="w-full p-4 sm:p-6">
      <!-- Voucher Details -->
      <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-y-6 gap-x-6 sm:gap-x-10 mb-6">
        <div>
          <p class="text-gray-500 text-sm mb-1">{{ useLocalization('voucher.details.field.name') }}</p>
          <p class="font-semibold text-black break-words">{{ voucherView_voucher?.data.name }}</p>
        </div>

        <div>
          <p class="text-gray-500 text-sm mb-1">{{ useLocalization('voucher.details.field.validity') }}</p>
          <p class="text-black">{{ startPeriod }} - {{ endPeriod }}</p>
        </div>
        ...
        <div>
          ...
          <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-black">
            <!-- Diskon Persen -->
        ...
        <div>
          ...
          <p class="text-black">
            {{
              voucherView_voucher?.data.isApplyAllProducts
                ? 'All Product'
                : useLocalization('voucher.details.field.selected-products')
            }}
          </p>
        </div>
      </div>
      ...
      <!-- Product Table -->
      <div v-if="voucherView_voucher?.data.isApplyAllProducts == false" class="mt-6 overflow-x-auto">
        <AppBaseDataTable
          :columns="PRODUCT_SELECTED_LIST_COLUMNS"
          header-title="Product Selected"
          :data="voucherView_voucher?.data.voucherHasProducts"
          :is-using-custom-body="true"
          :is-using-pagination="false"
          :is-using-header="false"
          :is-using-filter="false"
          class="border border-gray-200 rounded-md overflow-hidden min-w-[500px]"
        >
          <template #body="{ column, data }">
            ...
            <template v-else-if="column.value === 'category'">
              <span>{{
                data.products?.categoriesHasProducts.map((item: any) => item.categories.category).join(', ') || '-'
              }}</span>
            </template>
      ...
      <div v-if="!voucherView_voucher?.data.isApplied" class="mt-4">
        <button
          class="flex items-center justify-center gap-2 px-4 py-2 border border-primary text-primary rounded-md hover:bg-blue-50 transition w-full sm:w-auto"

...
watch(isAutoGenerateCode, val => {
  if (val) {
    // auto generate → kosongkan brandCode dan anggap valid
    brand_formData.value.brandCode = '';
    autoGeneratedCodeValid.value = true;
  } else {
    // manual input → harus ada isi
    autoGeneratedCodeValid.value = brand_formData.value.brandCode !== '';
  }
});

// cek kondisi ketika user isi brandCode manual
watch(
  () => brand_formData.value.brandCode,
  code => {
...
watch(
  ...
  ([mode, item]) => {
    if (mode === 'edit' && item) {
      ...
      brand_formData.value.brandCode = item.brandCode ?? '';
    } else {
        ...
        <section class="flex flex-col gap-4 w-full">
          ...
          >
            <div class="flex items-center gap-3">
              <PrimeVueInputText
                id="brand-code"
                v-model="brand_formData.brandCode as any"
                type="text"

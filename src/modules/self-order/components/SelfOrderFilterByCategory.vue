 ...
 */
const { selfOrder_productState, selfOrder_handleSelectCategory } =
...
<template>
  <section id="self-order-filter-by-category" class="hidden lg:flex flex-col gap-4 mb-6 mt-2">
    <h2 class="font-normal text-xs text-text-disabled">
      {{ useLocalization('cashier.mainSection.filterByCategory') }}
    </h2>

    <section
      id="self-order-filter-by-category-card"
      ref="containerRef"
    ...
    >
      <PrimeVueCard
        ...
        @click="selfOrder_handleSelectCategory(category.id)"
      >
        <template #content>
          <section id="self-order-card-content" class="flex flex-col gap-3">
            <AppBaseImage
      ...
      <PrimeVueCard
        ...
        @click="selfOrder_handleSelectCategory('bundle')"
      >
        <template #content>
          <section id="self-order-card-content" class="flex flex-col gap-3 items-center">
            <AppBaseImage
...
<style scoped>
#self-order-filter-by-category-card {

 ...
 */
const { selfOrder_modalVoucher, selfOrder_handleVoucher } =
  inject<ISelfOrderProvided>('selfOrder')!;

const localSearch = ref('');

watch(
  () => selfOrder_modalVoucher.value.data,
  (newVal) => {
    if (newVal && newVal.length > 0) {
      const isSelectedVoucherStillAvailable = newVal.find(
        (f) => f.id === selfOrder_modalVoucher.value.form.voucherId && f.available,
      );
      ...
      if (first) {
        selfOrder_modalVoucher.value.form.voucher_code = first.code;
        selfOrder_modalVoucher.value.form.voucherId = first.id;
      } else {
...
<template>
  <section v-if="voucherPermission" id="self-order-summary-modal-voucher" >
    <PrimeVueDialog
      v-model:visible="selfOrder_modalVoucher.show"
      modal
    ...
    >
      <template #container="{ closeCallback }">
        <section id="self-order-summary-modal-voucher-content" class="flex flex-col gap-6 p-6 h-full">
          <section
            id="self-order-summary-modal-voucher-content-body"
          ...
          >
            ...
            <section
              ...
              :class="{ 'justify-center items-center': !selfOrder_modalVoucher.data.length }"
            >
              <span v-if="!selfOrder_modalVoucher.data.length" class="text-text-disabled">{{
                useLocalization('cashier.orderSummary.voucher.noDataVoucher')
              }}</span>

              <div
                v-for="(item, index) in selfOrder_modalVoucher.data"
                v-else
                :key="index"
                ...
                :class="{
                  ...
                  'border border-primary-border bg-primary-background':
                    selfOrder_modalVoucher.form.voucher_code === item.code,
                }"
                @click="
                  () => {
                    if (!item.available) {
                      selfOrder_modalVoucher.form.voucher_code = ''
                      selfOrder_modalVoucher.form.voucherId = ''
                      return
                    }

                    if (selfOrder_modalVoucher.form.voucher_code === item.code) {
                      selfOrder_modalVoucher.form.voucher_code = ''
                      selfOrder_modalVoucher.form.voucherId = ''
                    } else {
                      selfOrder_modalVoucher.form.voucher_code = item.code
                      selfOrder_modalVoucher.form.voucherId = item.id
                    }
                  }
                "
              >
          ...
          <div class="flex justify-end gap-4">
            ...
            <PrimeVueButton
              ...
              @click="
                selfOrder_handleVoucher(selfOrder_modalVoucher.form.voucherId);
              "
            ></PrimeVueButton>
          </div>
        </section>

...
<template>
  <section id="self-order-summary-modal-select-table">
    <PrimeVueDialog
      v-model:visible="selfOrder_modalSelectTable.show"
      modal
      :style="{
    ...
    >
      <template #container="{ closeCallback }">
        <section id="self-order-summary-modal-select-table" class="flex flex-col gap-6 p-6 flex-1 min-h-0">
          <!-- Form -->
          <section id="self-order-summary-modal-select-table-form" class="flex flex-col gap-3">
            <section id="self-order-summary-modal-select-table-form-title" class="flex flex-col gap-2">
          ...
          <!-- Body -->
          <section id="self-order-summary-modal-select-table-body" class="gap-4 grid grid-cols-12 flex-1 min-h-0">
            <!-- Left Section -->
            <div
            ...
            >
              <div class="flex gap-4 items-center">
                ...
                <PrimeVueSelect
                  ...
                  option-value="value"
                  class="w-full"
                  :disabled="!selfOrder_modalSelectTable.listFloor.length"
                />
              </div>
              ...
              <div
                ...
                :class="{
                  'flex items-center justify-center h-full w-full ':
                    !selfOrder_getListActiveFloor.length,
                }"
              >
                <template v-if="accountStoreDetail_storeTables?.length">
                  ...
                  >
                    <AccountStoreTableLayout
                      v-model="selfOrder_modalSelectTable.selectedTable"
                      ...
                      :class="[
                        'flex gap-2 px-3 py-4 rounded-xl',
                        {
                          'cursor-pointer bg-primary-background border-primary-border border drop-shadow-xl drop-shadow-primary-background':
                            selfOrder_modalSelectTable.selectedTable.includes(childItem.name),
...
'cursor-not-allowed bg-grayscale-20 text-text-disabled border border-grayscale-20': childItem.statusTable === 'occupied',
                          ...
                          'cursor-pointer hover:bg-grayscale-10/25 border border-grayscale-10 hover:border-primary-border':
                            false && !selfOrder_modalSelectTable.selectedTable.includes(childItem.name),
                        },
                      ]"
                      @click="() => {
                        ...
                        selfOrder_onToggleSelectTable(childItem.name)
                      }"
                    >
                      <PrimeVueCheckbox
                        :model-value="selfOrder_modalSelectTable.selectedTable.includes(childItem.name)"
                        binary
                        :readonly="true"
                      ></PrimeVueCheckbox>
              ...
              <section
                v-if="selfOrder_modalSelectTable.selectedTable.length > 1"
          ...
          <div class="flex justify-end gap-2">
            ...
            <PrimeVueButton
              ...
              :disabled="!selfOrder_modalSelectTable.selectedTable"
              @click="
                selfOrder_modalSelectTable.show = false;
                selfOrder_onSelectTable();
              "
            ></PrimeVueButton>
          </div>

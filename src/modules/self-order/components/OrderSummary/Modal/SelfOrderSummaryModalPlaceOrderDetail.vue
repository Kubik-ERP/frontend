 ...
 */
const {
  selfOrder_handlePlaceOrderDetail,
...
const v$ = useVuelidate(rules, selfOrder_paymentForm, {
  $autoDirty: true,
});
...
const calculateChangeAmount = () => {
  const paymentAmount = selfOrder_paymentForm.paymentAmount || 0;
  const totalPrice = selfOrder_calculateEstimation.value.data.grandTotal || 0;
...
const handlePaymentInput = (event: { value: string | number | undefined }) => {
  ...
  selfOrder_paymentForm.paymentAmount = numericValue;
 ...
 */
watch(
  () => selfOrder_paymentForm.paymentAmount,
  () => {
    calculateChangeAmount();
  },
  { immediate: true },
 ...
 */
watch(
  () => selfOrder_calculateEstimation.value.data.grandTotal,
  () => {
    calculateChangeAmount();
  },
  { immediate: true },
...
<template>
  <section id="self-order-summary-modal-place-order-detail">
    <PrimeVueDialog
      v-model:visible="selfOrder_modalPlaceOrderDetail.show"
      modal
          ...
          >
            ...
            >
              ...
              <PrimeVueIconField class="flex w-full">
                ...
                <PrimeVueInputNumber
                  id="payment-amount"
                  v-model="selfOrder_paymentForm.paymentAmount"
            ...
            <section id="list-balances" class="flex flex-wrap items-center gap-2 w-full">
              <PrimeVueChip
                ...
                @click="
                  selfOrder_paymentForm.paymentAmount =
                    selfOrder_calculateEstimation?.data?.grandTotal;
                  calculateChangeAmount();
                "
              >
                <template #default>
                  <div class="flex items-center gap-2">
                    ...
                    <span class="font-semibold text-black text-xs">
                      {{
                        useCurrencyFormat({
                          data: selfOrder_calculateEstimation?.data?.grandTotal,
                          addSuffix: true,
                        })
                      }}
                    </span>
              ...
              <PrimeVueChip
                ...
                class="bg-secondary/10 cursor-pointer hover:bg-secondary basic-smooth-animation"
                @click="
                  selfOrder_paymentForm.paymentAmount = suggestionPrice;
                  calculateChangeAmount();
                "
              >
                <template #default>
            ...
            <div class="flex flex-col gap-2">
              <div class="flex justify-between">
                ...
                <span class="text-sm lg:text-base font-semibold">{{
                  useCurrencyFormat({
                    data: selfOrder_paymentForm.paymentAmount,
                  })
                }}</span>
              </div>
              <div class="flex justify-between">

...
<template>
  <section id="inventory-items-list-ui" class="flex flex-col gap-6 max-w-[85rem] w-full">
    <AppBaseDataTable
      ...
      <template #body="{ column, data }">
        ...
        <template v-else-if="column.value === 'expiryDate'">
          <span class="text-gray-700">
            {{ new Date(data.expiryDate).toLocaleDateString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric' }) }}
          </span>
        </template>

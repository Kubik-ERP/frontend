...
const { salesReport_columns, report_queryParams, report_getSalesReport, salesReport_salesByYear_values } =
  useReportService();
...
const formattedDataTable = () => {
  const newData =
    salesReport_salesByYear_values.value?.groupedSummary?.map(item => {
      return {
        group: item.group,
...
<template>
  <section class="flex flex-col gap-4">
    <SummaryReport :summary="salesReport_salesByYear_values?.overallSummary" />
    <AppBaseDataTable
    ...
    >
      ...
      <template #filter>
        <CustomDatePicker
          ...
          @update:start-date="report_getSalesReport('year')"
        />
      </template>
    </AppBaseDataTable>
  </section>

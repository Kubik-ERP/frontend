    ...
    >
      <template #header-prefix>
        <h1 class="font-bold text-2xl text-text-primary">
          {{ useLocalization('reports.sales.customer.title') }}
        </h1>
      </template>
        ...
        >
          <section id="popover-content" class="flex flex-col">
            <PrimeVueButton
              ...
              @click="report_downloadPDF('advanced-sales-report', 'customer')"
            />
            <PrimeVueButton
              class="w-full text-black font-normal px-4 py-3"
              variant="text"
      ...
      <template #filter>
        <section class="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-4 pt-4">
          <CustomDatePicker
            ...
            @update:end-date="report_getSalesReport('customer')"
          />
          <PrimeVueSelect
            ...
            @change="report_getSalesReport('customer')"
          >

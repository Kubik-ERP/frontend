...
const { salesReport_columns, report_queryParams, report_getSalesReport, salesReport_salesByQuarter_values } =
  useReportService();
...
const formattedDataTable = () => {
  const newData =
    salesReport_salesByQuarter_values.value?.groupedSummary?.map(item => {
      return {
        group: item.group,
...
<template>
  <section class="flex flex-col gap-4">
    <SummaryReport :summary="salesReport_salesByQuarter_values?.overallSummary" />
    <AppBaseDataTable
    ...
    >
      <template #header-prefix>
        <h1 class="font-bold text-2xl text-text-primary">Sales By Quarter</h1>
      </template>
      ...
      <template #filter>
        <CustomDatePicker
          ...
          @update:start-date="report_getSalesReport('quarter')"
        />
      </template>
    </AppBaseDataTable>
  </section>

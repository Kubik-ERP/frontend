...
const formattedDataTable = () => {
  const newData =
    loyaltyPointReport_spendBased_values?.value?.table?.map(item => {
      return {
        invoiceId: `#${item.invoiceId}`,
        purchaseDate: useFormatDate(item.purchaseDate, 'dd/MMM/yyyy'),
        customer: item.customer,
        grandTotal: useCurrencyFormat({ data: item.grandTotal }),
        orderType: useTitleCaseWithSpaces(item.orderType),
        totalPointsEarned: useCurrencyFormat({ data: item.totalPointsEarned, hidePrefix: true }),
        pointExpiryDate: useFormatDate(item.pointExpiryDate, 'dd/MMM/yyyy'),
      };
    }) || [];
  return newData;
};
          ...
          <tbody>
            <tr class="bg-secondary/10">
              ...
              <td class="text-right p-1.5">
                {{
                  useCurrencyFormat({
                    data: loyaltyPointReport_spendBased_values?.dashboard?.sumOfAllPoints,
                    hidePrefix: true,
                  })
                }}
              </td>
            ...
            <tr>
              ...
              <td class="text-right p-1.5">
                {{
                  useCurrencyFormat({
                    data: loyaltyPointReport_spendBased_values?.dashboard?.sumOfAllPointsExpired,
                    hidePrefix: true,
                  })
                }}
              </td>
            ...
            <tr class="bg-secondary/10">
              ...
              <td class="text-right p-1.5">
                {{
                  useCurrencyFormat({
                    data: loyaltyPointReport_spendBased_values?.dashboard?.totalCustomers,
                    hidePrefix: true,
                  })
                }}
              </td>
            ...
            <tr>
              ...
              <td class="text-right p-1.5">
                {{
                  useCurrencyFormat({
                    data: loyaltyPointReport_spendBased_values?.dashboard?.totalInvoices,
                    hidePrefix: true,
                  })
                }}
              </td>

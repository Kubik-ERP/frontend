...
const formattedDataTable = () => {
  const newData = inventoryReport_itemPerformance_values?.value?.map(item => {
    return {
      itemName: item.itemName,
      stockQty: useCurrencyFormat({ data: item.stockQty || 0, hidePrefix: true }),
...
const handleExportToCsv = () => {
  exportToCsv({
    ...
    columns: inventoryReport_itemPerformance_columns,
        ...
        >
          <section id="popover-content" class="flex flex-col">
            <PrimeVueButton
              ...
              @click="report_downloadPDF('inventory-report', 'item-performance')"
            />
            <PrimeVueButton
              class="w-full text-black font-normal px-4 py-3"
              variant="text"
      ...
      <template #filter>
        <section class="grid grid-cols-1 xl:grid-cols-2 gap-4 pt-4">
          <CustomDatePicker
            ...
            class=""
            @update:end-date="report_getInventoryReport('item-performance')"
          />
          <PrimeVueSelect

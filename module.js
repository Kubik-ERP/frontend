...
if (args.length === 0) {
  ...
  const kebabModuleName = moduleName.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
    ...
    listFolderNames.forEach(folderName => {
      ...
      switch (folderName) {
        ...
        case 'routes':
          ...
          break;
        case 'services':
          fs.closeSync(fs.openSync(path.join(modulePath, folderName, `${kebabModuleName}.service.ts`), 'w'));
          break;
        case 'views':
